...
export default {
  mounted() {
    ...
    // 定义数据
    let dataset = [40, 80, 50, 60, 10, 100, 20, 70];
    let padding = { top: 20, left: 20, bottom: 20, right: 20 },
      width = 800 - padding.left - padding.right,
      height = 600 - padding.top - padding.bottom
    // 添加坐标轴
    let g = svg.append('g').attr('transform', `translate(${padding.top}, ${padding.left})`)
    // xy比例尺
    // d3.range(dataset.length)  dataset 数组下标集合的一个数组
    let xScale = d3.scaleBand().domain(d3.range(dataset.length)).range([0, width]).padding(0.1)
    // let xScale = d3.scaleBand().domain(dataset).rangeRound([0, width]).padding(0.1)
    let yScale = d3.scaleLinear().domain([0, d3.max(dataset)]).range([height, 0])
    // xy轴
